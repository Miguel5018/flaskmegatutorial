...
def get_users():
    ...
    per_page = min(request.args.get("per", type=int), 100)
    return User.to_collection_dict(sa.select(User), page, per_page, "api.get_user")


@bp.route("/users/<init:id>/followers", methods=["GET"])
...
@bp.route("/users/<init:id>/following", methods=["GET"])
def get_following(id):
    user = db.get_or_404(User, id)
    page = request.args.get("page", 1, type=int)
    per_page = min(request.args.get("per_page", type=int), 100)
    return User.to_collection_dict(
